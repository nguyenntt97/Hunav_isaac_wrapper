...
if [ "$1" = "--help" ] || [ "$1" = "-h" ]; then
    ...
    echo "                                           #   Overrides an authored navmesh, so the run is no"
    echo "                                           #   longer on the mesh the scenario was validated on"
    echo "  HUNAV_NAVMESH_IGNORE_AUTHORED=1          # Ignore the scenario's assigned navmesh and derive"
    echo "                                           #   the volume from the agents (pre-assignment behaviour)"
    echo "  HUNAV_NAVMESH_USEGPU=0|1                 # GPU navmesh path (default: plugin default, on)"
    echo "  HUNAV_GO2_POLICY=off                     # Measurement: disable the Go2 locomotion policy"
    echo "                                           #   The robot holds its spawn stance and does NOT walk;"
    echo "                                           #   use only to price the policy out of a frame profile"
